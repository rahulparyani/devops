...
appPath="/data/tomcat/"+app+"/"
wildFlyVersion="wildfly-9.0.1.Final"
deployments=appPath+"webapps/"
indexPath=deployments+"monitor/"
conf=appPath+"conf/"
...
if ((willDeployExecutable=="true") or (willDeployProperties=="true") or (willRestart=="true") or (willDeployStatic=="true") or (willDeployConf=="true") or (willDeployConfiguration=="true") or (willDeployLib=="true") or (willDeployDynamicJars=="true")):
    ...
    def copyStatic():
        ...
        try:
            p=subprocess.run(["scp","-i","/etc/ansible/"+prvfle+"","-r",src,static], stderr=subprocess.PIPE, universal_newlines=True)
            if p.returncode == 0:
    ...
    def deleteTemp():
        sys.stdout.write ('In deleteTemp method\n')
        now = datetime.now()
        print("now =", now)
        try:
            p=subprocess.run('rm -fr '+tmp+' ', shell=True, stderr=subprocess.PIPE, universal_newlines=True)
            if p.returncode == 0:
                print("temp deleted")
            else:
                print("temp not exists", p.stderr)
        except OSError as e:
            print("Problem occured while deleting temp", p.stderr)
            print(sys.stderr, "Execution failed:", e)
        sys.stdout.write ('Exit from deleteTemp method\n')
    ...
    def deleteWork():
        ...
        print("now =", now)
        try:
            p=subprocess.run('rm -fr '+work+' ', shell=True, stderr=subprocess.PIPE, universal_newlines=True)
            if p.returncode == 0:
                print("temp deleted")
            else:
                print("temp not exists", p.stderr)
